...
| `example-transfer.xml` | Worked, schema-valid example of the **transfer** profile (identity + keys), unsigned. |
| `example-shipment.xml` | The same two devices in the **shipment** profile (adds `ManufacturingInfo`), unsigned. |
| `example-*-signed.xml` | The signed counterparts of the two files above (enveloped `ds:Signature`). |
| `sample/` | Python reference app to generate, validate, and import shipment files. See [`sample/README.md`](sample/README.md). |
| `README.md` | This document. |

The four example files are generated by the sample app, not hand-maintained — regenerate them
with `uv run --project sample python sample/main.py` so they always reflect what the builder
produces. See [`sample/README.md`](sample/README.md) for the validator and importer too.
